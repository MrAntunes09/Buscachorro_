...
function ObterSenhaUsuario($id_usuario)
{
    $sql = '
        SELECT nm_senha
        FROM tb_usuario
    ...
    if ($res && $res->num_rows > 0) {
        $dadosUsuario = $res->fetch_assoc();
        return $dadosUsuario['nm_senha'];
    }
...
{
    $sql = '
        SELECT nm_nome, ct_usuario, nm_email, nm_cidade, nm_senha, nm_estado, nm_cep
        FROM tb_usuario
        WHERE cd_usuario = "' . $id_usuario . '";
    ';
